...
- **2D.4 — Supplier Analytics** (transparent, receipt-traceable supplier performance; [metric definitions](docs/analytics/SUPPLIER-PERFORMANCE-METRICS.md)):
  ...
  - **2D.4C — Supplier Analytics UX (deterministic-granularity time-series trends + coverage per bucket; metric evidence drill-down reconciling numerator/denominator to each displayed metric with receipt links and `cost.view`-gated price detail; KPI dashboard + sortable ranking where missing sorts last, not zero; scorecard trend charts + evidence panel):** ✅ [docs/PHASE-2D4C-SUPPLIER-UX.md](docs/PHASE-2D4C-SUPPLIER-UX.md)
- **✅ 2D.4 — Supplier Analytics complete.**
- **Next:** 2D.5 FIFO Costing (first-in-first-out valuation alongside the existing WAC model).
